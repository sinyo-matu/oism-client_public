...
import { deleteAlertOpenAtom, returnForDelete } from "../../store/returnList";
...
export const DeleteAlertModal = () => {
  ...
  const onConfirmClicked = async () => {
    setLoading(true);
    try {
      await privateApiCall(`/return/${item!.id}`, "DELETE");
    } catch (err) {
      switch (err) {
    ...
    setLoading(false);
    setIsOpen(false);
    toast.success(t("common.success.beenConcealed", { value: item!.returnNo }));
  };
  return (
    <ConfirmModal
      isOpen={isOpen}
